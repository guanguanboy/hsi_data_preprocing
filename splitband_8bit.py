...
splited_dataset_root_path = './band_splited_dataset_8bit'
...
for i in range(len(img_name_list_15ms)):
    ...
    for i in range(200):
        band_idx = 200 + i
        single_band_noise_img = spectral_img_1ms.read_band(band_idx)
        single_band_noise_img8bit = single_band_noise_img.astype(np.float)
        single_band_noise_img8bit = (single_band_noise_img8bit / 2047) * 255 #归一化，将12bit数据转换为16bit数据
        single_band_noise_img8bit = single_band_noise_img8bit.astype(np.uint8)
        im = Image.fromarray(single_band_noise_img8bit)

        image_name = splited_dataset_noise_path + str(image_count) + '.png'
        im.save(image_name)


        single_band_clean_img = spectral_img_15ms.read_band(band_idx)
        single_band_clean_img8bit = single_band_clean_img.astype(np.float)
        single_band_clean_img8bit = (single_band_clean_img8bit / 2047) * 255 #归一化
